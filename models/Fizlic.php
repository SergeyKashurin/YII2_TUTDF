...
class Fizlic extends ActiveRecord
{
    public $counts;
    ...
    public function rules()
    {
        ...
        return[
            ...
            [['PoruchFlag', 'PoruchObesb', 'PoruchSumm'],'string'],
            ...
            [['PersonaFam', 'PersonaName', 'PersonaOtc', 'PersonaBorn', 'PersonaBornHome', 'PersonaInn', 'PasportSer',
                'PasportNumb', 'PasportDate', 'PasportWhere', 'PostIndex', 'HomeArea', 'HomeTown', 'HomeStreetType', 'HomeStreet', 'HomeHouse', 'PersonaTel1', 'PersonaTel1Type',
                'KredScet', 'KredScetType', 'KredScetOtn', 'KredOpenDate', 'KredLastPay', 'KredScetActive', 'PersonaTel2Type',
                'KredFullSum', 'KredSum', 'KredEndDate', 'KredEndPay', 'KredEndProc', 'formatTUTDF', 'operationType', 'KodZaloga', 'PSK'
            ...
            #[['KredProc', 'MonthCount'], 'match', 'pattern' => '/^[0-9]{1,2}$/i'],
            [['KredScet'], 'match', 'pattern' => '/^[0-9]{20}$/i'],
     ...
     * @var $content array
     */
    public function generateFizlic($counts)
    {
        ...
        if (!file_exists("FIZLIC/$this->nowDate" . "_" . $fname)) {
            mkdir("FIZLIC/$this->nowDate" . "_" . $fname, 0777);
        ...
        for($i=0; $i<=$counts+1; $i++) {
            ...
            if ($this->formatTUTDF === "0" || !isset($this->formatTUTDF)) {
                $this->TR01 = "TR01\tNAME\t$this->KredScet\t$this->KredScetType\t$this->KredScetOtn\t$this->KredOpenDate\t$this->INCREMENTKredLastPay\t$this->KredScetActive\t$this->dateStatusScet\t$this->INCREMENTKredLastPay\t$this->KredFullSum\t$this->KredPogashenieSum\t0\t \t \t1\tRUB\t \t$this->KredEndDate\t$this->KredEndPay\t$this->KredEndProc\t \t \t \t$this->KredZadolgennost";
            } else if ($this->formatTUTDF = '1') {
            ...
            file_put_contents("FIZLIC/$this->nowDate" . "_" . $fname . "/NAME_" . $this->nowDate . "_" . $this->nowTime . "$q", iconv('UTF-8', 'CP1251', $this->record));
        }
        }
    }
}